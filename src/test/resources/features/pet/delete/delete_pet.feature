Feature: Delete pet in pet store
  ...
  Scenario: delete pet happy path 200
    # preparation
    * def auth = 'oauth2_valid_token'
    * def authHeader = call read('classpath:karate-auth.js') auth
    * def body = karate.readAsString('classpath:features/pet/create/test-data/create_pet_schema.json')
    ...
    * string created_pet_id_str = created_pet_id
    * match response.code == 200
    * match response.type == 'unknown'
    * match response.message == created_pet_id_str

  Scenario: delete not existing pet 404
    # preparation
    * def auth = 'oauth2_valid_token'
    * def authHeader = call read('classpath:karate-auth.js') auth
    * def body = karate.readAsString('classpath:features/pet/create/test-data/create_pet_schema.json')
